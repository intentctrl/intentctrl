...
The chat interface is the primary way users interact with the AI. All functionality is available through the `useIntentCtrlChat()` hook.
...
export function ChatWindow() {
  const { messages, sendMessage, status, stop, error } = useIntentCtrlChat();
