...
const CopyButton = ({ content, iconSize = 14, className, ...props }: CopyButtonProps) => {
  ...
  return (
    ...
    >
      {isCopied ? (
        <IconCheck size={iconSize} className="animate-in zoom-in-50 text-green-900 duration-200 dark:text-green-400" />
      ) : (
        <IconCopy size={iconSize} className="animate-in zoom-in-50 duration-200" />
      )}
    </button>
  );
};
